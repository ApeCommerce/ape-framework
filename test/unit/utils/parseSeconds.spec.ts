...
describe('Parsing seconds', () => {
  test('Returns expected value', async () => {
    ...
    expect(parseSeconds({ foo: 'foo' })).toBe(0);
    expect(parseSeconds(['one', 'two'])).toBe(0);
    expect(parseSeconds(() => 'foo')).toBe(0);
    ...
    expect(parseSeconds('0ms')).toBe(0);
    ...
    expect(parseSeconds('3000mss')).toBe(3);
    expect(parseSeconds('3000millisecond')).toBe(3);
